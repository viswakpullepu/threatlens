...
- **Global scope**: `.agents/plugins/get-shit-done/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `.agents/plugins/get-shit-done/notes/`)
...
2. Glob `.agents/plugins/get-shit-done/notes/*.md` (if directory exists) — global notes
...
Global (.agents/plugins/get-shit-done/notes/):
...
2. **No `.planning/`**: Falls back to global `.agents/plugins/get-shit-done/notes/` — works in any directory
