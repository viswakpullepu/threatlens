...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `.agents/plugins/get-shit-done/.gsd-surface.json`
(sibling to `.agents/plugins/get-shit-done/.gsd-profile`) and re-stages the active skills directory in place.
Skill dirs live at `.agents/plugins/get-shit-done/skills/gsd-*/`.
...
(`.agents/plugins/get-shit-done`), NOT the skills sub-directory (`.agents/plugins/get-shit-done/skills`).

This matches `installRuntimeArtifacts` and `uninstallRuntimeArtifacts`, which also
receive `.agents/plugins/get-shit-done` as `configDir`. The skill dirs themselves live at
`.agents/plugins/get-shit-done/skills/gsd-*/` because the `claude global` layout has `destSubpath =
'skills'` — they are derived from `configDir`, not the root for it.

```bash
# Claude Code — global install
RUNTIME_CONFIG_DIR="${CLAUDE_CONFIG_DIR:-.agents/plugins/get-shit-done}"
SCOPE="global"
...
(i.e. `.agents/plugins/get-shit-done/.gsd-surface.json`).
...
<execution_context>
Surface state file: `.agents/plugins/get-shit-done/.gsd-surface.json`
Install profile marker: `.agents/plugins/get-shit-done/.gsd-profile`
Skill dirs: `.agents/plugins/get-shit-done/skills/gsd-*/`
Engine module: `.agents/plugins/get-shit-done/get-shit-done/bin/lib/surface.cjs`
Cluster definitions: `.agents/plugins/get-shit-done/get-shit-done/bin/lib/clusters.cjs`
</execution_context>
